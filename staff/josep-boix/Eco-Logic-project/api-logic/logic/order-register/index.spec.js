...
describe ('logic - place order', () => {
    ...
    it('should fail on empty userId', () =>{
        userId = ""
        expect(() => registerOrder(userId)).to.throw('userId is empty or blank')
    })

    it('should fail on undefined userId', () => {
        userId = undefined
        expect(() => registerOrder(userId)).to.throw(`userId with value undefined is not a string`)
    })

    it('should fail on wrong data type for userId', () => {
        userId = 123
        expect(() => registerOrder(123)).to.throw(`userId with value 123 is not a string`)
    })
